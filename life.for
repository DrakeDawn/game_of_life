...
C * Assignment 1
C * Name : Fu Xinyu
C * Student ID : 1155092193
C * Email Addr : xyfu6@cse.cuhk.edu.hk
C */

C Fortran program to simulate Game of Life
      PROGRAM Gameoflife
        integer fdin, fdout, gen, row, col, stillgen, i, ios, index
        character*90 name
        character*80 pattern1(100), pattern2(100)
        logical still
        character*5 intholder
        fdin = 1
        fdout = 2
        call infopen(fdin)
        read(fdin,'(A)') name
...
C       write result statement into file
   60   if ( .NOT. still .AND. (gen .EQ. 1) ) GO TO 70
        ...
        write(intholder,'(I5)') stillgen
        call getindex(intholder, index)
        write(fdout, '(A,X,A,X,A,A)')
     +  'It is a still life after',intholder(index:5),
     +  'steps.',char(13)
        GO TO 110
   70   write(fdout, '(A,X,I1,X,A,A)')
     +  'It is still not a still life even after',1,'step.',char(13)
        GO TO 110
   80   write(intholder,'(I5)') gen
        call getindex(intholder, index)
        write(fdout, '(A,X,A,X,A,A)')
     +  'It is still not a still life even after',
     +  intholder(index:5),'steps.',char(13)
        GO TO 110
   90   write(fdout, '(A,A)')
     +  'It is a still life initially.',char(13)
        GO TO 110
  100   write(fdout, '(A,X,I1,X,A,A)')
...
C function that returns the length of a string
  ...
  260   length = i
      ...
      SUBROUTINE getindex(num, index)
        character*(*) num
        integer i, index
        i = 1
  270   if ( i .GT. LEN(num) ) GO TO 280
          if ( num(i:i) .NE. ' ' ) GO TO 280
          i = i + 1
          GO TO 270
  280   index = i
      END
